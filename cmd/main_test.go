...
func (api *apiFeature) theResponseShouldBe(arg1 string) error {
	...
	resp, err := api.client.R().SetHeader("Content-Type", "application/json").SetQueryParams(
		map[string]string{
	...
	if res.Translation != arg1 {
		return fmt.Errorf("translation %s should be set to %s result %s", url, arg1, res.Translation)
	}
